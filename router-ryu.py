# Copyright (C) 2020 DANG QUANG KIEU
# Integrating firewall into ryu router application
# It is freely for using routing function under Apache License; however,
# the firewall function is DANGQUANGKIEU proprietary and you are not allowed to use any part of code until
# you are authorised
# Special thanks to Nippon Telegraph and Telephone Corporation for the router application
...
REST_FIREWALL = 'firewall'
REST_FIREWALL_RULE_ID = 'rule_id'
...
class RestRouterAPI(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        mapper.connect('router', path, controller=RouterController,
            ...
            # For firewall
        path = '/router/firewall/{switch_id}'
        mapper.connect('router', path, controller=RouterController,
                       requirements=requirements,
                       action='get_fire_rules',
                       conditions=dict(method=['GET']))
        # For vlan data
        ...
        mapper.connect('router', path, controller=RouterController,
                       ...
                       conditions=dict(method=['DELETE']))
        path = '/router/{switch_id}/vlanlist'
        mapper.connect('router', path, controller=RouterController,
                       requirements=requirements,
                       action='get_vlan_list',
                       conditions=dict(method=['GET']))
        # for general troubleshoot
        path = '/rou/listdpid'
        mapper.connect('rou', path, controller=RouterController,
                       action='get_dpid',
                       conditions=dict(method=['GET']))
...
class RouterController(ControllerBase):
    ...
    def register_router(cls, dp):
        ...
        cls.router_list.setdefault(dp.id, router)
    ...
    # GET /router/listdpid
    @rest_command
    def get_dpid(self, req, **_kwargs):
        print("get dpid called")
        return self._list_routerinfo()
        # GET /router/listdpid

    # GET /firewall/{switch_ID}
    @rest_command
    def get_fire_rules(self, req, switch_id):
        print("get firewall called")
        return self._get_fire_rules(switch_id, VLANID_NONE, 'get_firewall_data')
    ...
    # GET /router/{switch_id}/vlanlist
    @rest_command
    def get_vlan_list(self, req, switch_id, **_kwargs):
        return self._access_router(switch_id, VLANID_NONE,
                                   'get_data', req)
    ...
    def _list_routerinfo(self):
        rest_message = []
        routers = self._get_router('all')
        for router in routers.values():
            function = getattr(router, 'get_data')
            # data = function()
            rest_message.append(router.sw_id)
        return rest_message

    def _get_fire_rules(self, switch_id, vlan_id, func):
        rest_message = []
        routers = self._get_router(switch_id)
        for router in routers.values():
            function = getattr(router, 'get_firewall_data')
            print("get firewall rest called")
            data = function(vlan_id)
            rest_message.append(data)
        return rest_message

    def _set_fire_rules(self):
        rest_message = []
        routers = self._get_router('all')
        for router in routers.values():
            function = getattr(router, 'get_data')
...
class Router(dict):
    ...
    def get_firewall_data(self, vlan_id):
        vlan_routers = self._get_vlan_router(vlan_id)
        if vlan_routers:
            msgs = [vlan_router.get_firewall_data() for vlan_router in vlan_routers]
        else:
            msgs = [{REST_VLANID: vlan_id}]
    ...
    def set_data(self, vlan_id, param, waiters):
        ...
        for vlan_router in vlan_routers:
            try:
                msg = vlan_router.set_data(param, vlan_id)
    ...
    def delete_data(self, vlan_id, param, waiters):
        ...
        if vlan_routers:
            for vlan_router in vlan_routers:
                ...
                print("delete data on router class")
                if msg:
                    msgs.append(msg)
...
class VlanRouter(object):
    ...
    def delete(self, waiters):
        ...
        for msg in msgs:
            for stats in msg.body:
                ...
                print('Vlan list', self.vlan_id)
                if vlan_id == self.vlan_id:
                    self.ofctl.delete_flow(stats)
                else:
                    print("vlan not exist")  # Remove in production
    ...
    def _cookie_to_id(id_type, cookie):
        if id_type == REST_VLANID:
            ...
            assert id_type == 'rule_id'
            rest_id = (cookie & UINT32_MAX) >> 8
    ...
    def _id_to_cookie(self, id_type, rest_id):
        ...
        elif id_type == 'rule_id':
            cookie = vid + (rest_id << COOKIE_SHIFT_FIREWALLID)
    ...
    def get_data(self):
        ...
        if routing_data[REST_ROUTE]:
            print("sw_id with routing table ", self.routing_tbl)
    ...
    def get_firewall_data(self):
        data = {}
        address_data = self._get_firewall_data()
    ...
    def _get_firewall_data(self):
        ...
        for key, value in self.rule_data.items():
            ...
            firewall_rule.append(data)

        print("Get ", firewall_rule)
        return {'Rule': firewall_rule}
    ...
    def set_data(self, data, vlan_id):
        details = None
        ...
        try:
            ...
            # Set routing data
            ...
            elif REST_FIREWALL in data:
                # rule_data = self.rule_data.add(data['src_address'], data['dst_address'])

                src_address = data['src_address']
                dst_address = data['dst_address']
                src_port, dst_port, nw_proto = None, None, None
                if 'nw_proto' in data:
                    if data['nw_proto'] == 'TCP':
                        nw_proto = inet.IPPROTO_TCP
                    elif data['nw_proto'] == 'UDP':
                        nw_proto = inet.IPPROTO_UDP
                    elif data['nw_proto'] == 'ICMP':
                ...
                print("value firewall is ", self.rule_data.get())
                rule_status = self._set_firewall_rule(src_address, dst_address, nw_proto=nw_proto, src_port=src_port,
                                                      dst_port=dst_port)
                details = 'Add firewall success {} '.format(rule_status)
    ...
    def _set_address_data(self, address):
        ...
        # vlan = self.vlan_data.add(self.vlan_id)
    ...
    def _set_firewall_rule(self, src, dst, nw_proto=None, src_port=None, dst_port=None):
        src_ip, src_mask, dummy = nw_addr_aton(src)
        dst_ip, dst_mask, dummy1 = nw_addr_aton(dst)
        ...
        if src_ip == dst_ip:
            ...
            rule = self.rule_data.add(src_ip, src_mask,
                                      dst_ip, dst_mask,
                                      nw_proto, src_port, dst_port)

            cookie = self._id_to_cookie('rule_id', rule_id)
            print("cookie injected ", cookie)
            self.ofctl.set_routing_flow(cookie, priority, None, dl_vlan=self.vlan_id,
                                        nw_src=src_ip, src_mask=src_mask,
                                        nw_dst=dst_ip, dst_mask=dst_mask,
                                        dec_ttl=False, nw_proto=nw_proto,
    ...
    def _set_vlan_data(self, vlan_id):
        vlan_list = self.vlan_data.add(vlan_id)
        return vlan_list

    def delete_data(self, data, waiters):
        print("delete data in vlan object ", data, waiters)
        ...
        elif REST_FIREWALL in data and 'rule_id' in data:
            msg = self._delete_rule_data(data['rule_id'], waiters)
            cookie = VlanRouter._cookie_to_id('rule_id', int(data['rule_id']))
            print ("msg for user ", msg)
        else:
            raise ValueError('Invalid parameter.')
    ...
    def _delete_rule_data(self, rule_id, waiters):
        if rule_id != REST_ALL:
            try:
                rule_id = int(rule_id)
            except ValueError as e:
                err_msg = 'Invalid [%s] value. %s'
                raise ValueError(err_msg % ('Rule ', e.message))
        # Get all flow.
        msgs = self.ofctl.get_all_flow(waiters)
        ...
        for msg in msgs:
            for stats in msg.body:
                ...
                if vlan_id != self.vlan_id:
                    continue
                rule_cookie_id = VlanRouter._cookie_to_id('rule_id', stats.cookie)

                if rule_id == REST_ALL:
                    if rule_id == COOKIE_DEFAULT_ID:
                        continue
                elif rule_id != rule_cookie_id:
        ...
        for flow_stats in delete_list:
            self.ofctl.delete_flow(flow_stats)
            rule_deleted = self.rule_data.delete(rule_id)
            delete_ids.append(rule_deleted)
        ...
        msg = {}
        if delete_ids:
            delete_ids = ','.join(str(rule_id) for rule_id in delete_ids)
            details = 'Delete rule [rule_id=%s]' % delete_ids
            msg = {REST_RESULT: REST_OK, REST_DETAILS: details}
    ...
    def _packetin_arp(self, msg, header_list):
        ...
        if src_ip == dst_ip:
            ...
            if (dst_addr is not None and
                ...
                #     print("Set packetin flow called with actions {} and type {} and macdest {}".
                #           format(actions, dl_type, dl_dst))
                cookie = self._id_to_cookie(REST_ADDRESSID, dst_addr.address_id)
                print("Preparing for modify flow cookie {}, dst addrs {} ".format(dst_ip, src_ip))

                self.ofctl.set_packetin_flow(cookie=cookie, priority=100, dst_ip=dst_ip, dst_mask=dst_addr.netmask
                                             , dl_type=2054, outport=in_port, dl_dst=header_list[ARP].src_mac)
                #                       dl_vlan=2,dst_ip=dst_addr,dst_mask=dst_addr.netmask)
                # actions = [self.dp.ofproto_parser.OFPActionOutput(port=2)]
                # self.set_flow(cookie, priority, dl_type=ether.ETH_TYPE_ARP,
                #             dl_vlan=3, nw_dst=dst_ip, dst_mask=dst_addr.netmask,
                #             actions=actions)
                self.ofctl.send_packet_out(in_port, output, msg.data)
                # print("ARP internal host ",in_port, header_list[ARP].src_mac)
                # print("mssg list ", header_list, header_list[VLAN].ethertype)
                print("Source mac ", self.port_data[in_port].mac)
    ...
    def _packetin_to_node(self, msg, header_list):
        ...
        if src_ip is not None:
            ...
            print("ARP flood ip ", src_ip, dst_ip)
        ...
        for key, value in self.routing_tbl.items():
            if value.gateway_ip == src_ip:
                ...
                self.logger.info('Set %s flow [cookie=0x%x]', log_msg, cookie,
                                 extra=self.sw_id)
                # if (value.dst_ip == '10.0.0.168'):
                #     print("*************routing table important***** ")
                #     self.ofctl.set_routing_flow(cookie, 109, None,
                #                                 dl_vlan=self.vlan_id,
                #                                 nw_src='10.0.0.192',
                #                                 src_mask=28,
                #                                 nw_dst=value.dst_ip,
        ...
        if src_ip not in gateways:
            ...
            if address is not None:
                ...
                self.logger.info('Set implicit routing flow [cookie=0x%x]',
                                 cookie, extra=self.sw_id)
                print("implicit routing flow was set on {}", format(self.sw_id))
                # self.logger.info('Output port is {}'.format(out_port))
                print("Outport ble ble ", out_port)
...
class RuleData(dict):
    ...
    def add(self, src_ip, src_mask, dst_ip, dst_mask, nw_proto=None, src_port=None, dst_port=None):
        if src_ip != dst_ip:
            rule = Rule(src_ip, src_mask, dst_ip, dst_mask, nw_proto, src_port, dst_port)
            print("Add vlan data called", self.get())
            key = self.rule_id
            self[key] = rule
            self.rule_id += 1
        else:
            msg = "Same dst and src"
...
class Rule(object):
    def __init__(self, src_ip, src_mask, dst_ip, dst_mask, nw_proto=None, src_port=None, dst_port=None):
    ...
    def get(self):
        ...
        # Join to human readable
        src_address = ''.join([str(self.src_ip), '/', str(self.src_mask)])
        dst_address = ''.join([str(self.dst_ip), '/', str(self.dst_mask)])

        if self.nw_proto is None:
            nw_proto = "all"

            return {"Source address ": src_address,
                    "Destination address ": dst_address,
                    "L4 Network protocol ": nw_proto}
        else:

            return {"Source address ": src_address,
                    "Destination address ": dst_address,
                    "L4 Network protocol ": self.nw_proto,
...
class OfCtl(object):
    ...
    def set_packetin_flow(self, cookie, priority, dl_type=0, dl_dst=0,
                          dl_vlan=0, dst_ip=0, dst_mask=32, nw_proto=0, outport=0):
...
class OfCtl_after_v1_2(OfCtl):
    ...
    def set_flow(self, cookie, priority, dl_type=0, dl_dst=0, dl_vlan=0,
                 nw_src=0, src_mask=32, nw_dst=0, dst_mask=32,
                 nw_proto=0, idle_timeout=0, actions=None, src_port=None, dst_port=None):
        ofp = self.dp.ofproto
        ofp_parser = self.dp.ofproto_parser
        cmd = ofp.OFPFC_ADD
        ...
        if dl_type:
            match.append_field(ofp.OXM_OF_ETH_TYPE, dl_type)
        # if dl_dst:
        #     match.append_field(ofp.OXM_OF_ETH_DST,dl_dst)
        # if dl_vlan:
        #     match.append_field(ofp.OXM_OF_VLAN_VID ,dl_vlan)
        if nw_src:
            match.append_field(ofp.OXM_OF_IPV4_SRC_W, ipv4_text_to_int(nw_src),
                               mask_ntob(src_mask))
        if nw_dst:
            match.append_field(ofp.OXM_OF_IPV4_DST_W, ipv4_text_to_int(nw_dst),
                               mask_ntob(dst_mask))
        if nw_proto:
            if dl_type == ether.ETH_TYPE_IP:
                match.append_field(ofp.OXM_OF_IP_PROTO, nw_proto)
                if nw_proto == inet.IPPROTO_TCP:
                    if src_port is not None:
                        match.append_field(ofp.OXM_OF_TCP_SRC, src_port)
                    if dst_port is not None:
                        match.append_field(ofp.OXM_OF_TCP_DST, dst_port)
                elif nw_proto == inet.IPPROTO_UDP:
                    if src_port is not None:
                        match.append_field(ofp.OXM_OF_UDP_SRC, src_port)
                    if dst_port is not None:
                        match.append_field(ofp.OXM_OF_UDP_DST, dst_port)
            elif dl_type == ether.ETH_TYPE_ARP:
                match.append_field(ofp.OXM_OF_ARP_OP, nw_proto)
    ...
    def set_routing_flow(self, cookie, priority, outport, dl_vlan=0,
                         nw_src=0, src_mask=32, nw_dst=0, dst_mask=32,
                         src_mac=0, dst_mac=0, idle_timeout=0, dec_ttl=False, nw_proto=None, src_port=None,
                         dst_port=None):
        ofp = self.dp.ofproto
        ofp_parser = self.dp.ofproto_parser

        dl_type = ether.ETH_TYPE_IP
        print("Discover structure ", nw_src, nw_dst, src_mask, dst_mask)
        actions = []
        if dec_ttl:
        ...
        self.set_flow(cookie, priority, dl_type=dl_type, dl_vlan=dl_vlan,
                      nw_src=nw_src, src_mask=src_mask,
                      nw_dst=nw_dst, dst_mask=dst_mask,
                      idle_timeout=idle_timeout, actions=actions, nw_proto=nw_proto, src_port=src_port,
                      dst_port=dst_port)
    ...
    def delete_firewall_flow(self, flow_stats):
        ofp = self.dp.ofproto
        ofp_parser = self.dp.ofproto_parser
        ...
        cookie = flow_stats.cookie
        cookie_mask = UINT64_MAX
        match = ofp_parser.OFPMatch(priority=150)
        inst = []
